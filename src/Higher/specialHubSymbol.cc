...
void
SpecialHubSymbol::compileEquations()
{
  FreeSymbol::compileEquations();
  setEqRewrite(&SpecialHubSymbol::eqRewrite);
}

bool
SpecialHubSymbol::eqRewrite(Symbol* symbol, DagNode* subject, RewritingContext& context)
{
  Assert(symbol == subject->symbol(), "bad symbol");
  SpecialHubSymbol* s = safeCastNonNull<SpecialHubSymbol*>(symbol);
